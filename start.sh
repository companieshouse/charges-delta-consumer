#!/bin/bash
#
# Start script for charges-delta-consumer
...
if [[ -z "${MESOS_SLAVE_PID}" ]]; then
    ...
    source ~/.chs_env/charges-delta-consumer/env
...
exec java ${JAVA_MEM_ARGS} -jar -Dserver.port="${PORT}" "${APP_DIR}/charges-delta-consumer.jar"