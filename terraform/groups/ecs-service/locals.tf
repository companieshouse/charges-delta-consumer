...
locals {
  ...
  service_name               = "charges-delta-consumer"
  container_port             = "8080"
  docker_repo                = "charges-delta-consumer"
  kms_alias                  = "alias/${var.aws_profile}/environment-services-kms"
  healthcheck_path           = "/charges-delta-consumer/healthcheck" # healthcheck path for charges-delta-consumer
  healthcheck_matcher        = "200"
  vpc_name                   = local.stack_secrets["vpc_name"]
  s3_config_bucket           = data.vault_generic_secret.shared_s3.data["config_bucket_name"]
  app_environment_filename   = "charges-delta-consumer.env"
